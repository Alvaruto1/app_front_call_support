...
import { getEmployee, getEmployees } from "./employeeSlice";
...
export default function FormSearch(props) {
  ...
  const okFunction = () => {
    Swal.fire({
      icon: "success",
      title: "Empleado(s) obtenido(s) correctamente",
  ...
  const onClickAll = (e) => {
    ...
    e.preventDefault();
    dispatch(getEmployees(okFunction, errorFunction));
  };
  const onClickSearch = (e) => {
    setActivedButton("search");
    e.preventDefault();
    dispatch(getEmployee(document_type_id, id, okFunction, errorFunction));
  };
  return (
    <div className="mt-4">
